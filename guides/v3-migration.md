...
## Upgrading Jido Action From Version 2 To Version 3

This guide contains the changes required to move an application from the
published `jido_action` `2.3.2` API to `3.0.0-beta.3`.

Each section starts with version 2 code that no longer has the same contract.
It then gives the version 3 replacement. New version 3 features that do not
require a change to version 2 code are outside the scope of this guide.
